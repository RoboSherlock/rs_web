...
public:
  ...
  void process(designator_integration_msgs::DesignatorResponse &designator_response, uint64_t timestamp)
  {
    ...
    try
    {
      ...
      if(timestamp != 0)
      {
        ...
        rs::QueryTimestamp ts = rs::create<rs::QueryTimestamp>(*cas);
        ts.timestamp.set(timestamp);
        sceneCas.set("QUERY_TIMESTAMP", ts);
      }
  ...
  // decide if the pipeline should be reset or not
  void process(std::vector<std::string> annotators, bool reset_pipeline_after_process, designator_integration_msgs::DesignatorResponse &designator_response, uint64_t timestamp)
  {
    process_mutex->lock();
    ...
    process(designator_response, timestamp);
    if(reset_pipeline_after_process)
    {
      resetPipelineOrdering();  // reset pipeline to default
    }
  ...
  {
    designator_integration_msgs::DesignatorResponse d;
    process(annotators, reset_pipeline_after_process, d,0);
  }
};
                               ...
                               designator_integration_msgs::DesignatorCommunication::Response &res, bool allSolutions)
  {
        ...
        it != bdgs.end(); it++)
    {
      ...
      if(engines.size() > 0)
      {
        ...
        engines.at(0).process(new_pipeline_order, true, designator_response, timestamp);
